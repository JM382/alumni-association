...
Page({
  ...
  onAlumniCardNoChange(e: WechatMiniprogram.CustomEvent) {
    this.setData({ 'form.alumniCardNo': e.detail.value })
  },

  onEnrollYearChange(e: WechatMiniprogram.CustomEvent) {
    this.setData({ 'form.enrollYear': e.detail.value })
  },

  onDepartmentChange(e: WechatMiniprogram.CustomEvent) {
  ...
  async onSubmit() {
    ...
    if (!form.alumniCardNo?.trim()) {
      wx.showToast({ title: '请填写校友卡号', icon: 'none' })
      return
    }
    if (!form.enrollYear?.trim()) {
    ...
    try {
      ...
      // 本地缓存一份资料，便于个人中心优先展示
      wx.setStorageSync('alumniCardNo', form.alumniCardNo.trim())
      wx.setStorageSync('enrollYear', form.enrollYear.trim())
      wx.setStorageSync('department', form.department.trim())
      wx.setStorageSync('phone', form.phone.trim())
      wx.setStorageSync('email', (form.email || '').trim())
